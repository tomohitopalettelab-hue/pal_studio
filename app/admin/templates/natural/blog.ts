...
export const naturalBlogTemplate: TemplateVariant = {
  id: 'variant-natural-blog',
  templateId: 'template-natural',
  name: 'Blog',
  pageSlug: 'blog',
  description: 'Natural / Blog list layout',
  html: `
<div class="template-root" style="--accent-color: #16a34a;">
  <div class="min-h-screen font-sans text-slate-900 bg-[#f7fdf7]">
    <header class="px-8 py-10 border-b border-emerald-200">
      ...
      <p class="text-sm text-emerald-500 mt-3">ブログ記事の一覧を表示するテストページです。</p>
    </header>
    <main class="max-w-5xl mx-auto px-6 py-12 space-y-8">
      <section id="top" class="grid gap-6 md:grid-cols-2">
        <article class="border border-emerald-200 rounded-2xl p-6 bg-white">
          ...
          <p class="text-sm text-emerald-500 mt-3">記事の概要がここに入ります。</p>
        </article>
        <article class="border border-emerald-200 rounded-2xl p-6 bg-white">
          <p class="text-xs uppercase tracking-widest text-emerald-400">2026.02.14</p>
